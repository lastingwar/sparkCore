...
import com.top10.{CategoryCountInfo, MyAcc}
import com.top10test1.UserVisitAction
...
object Day04Test5 {
  def main(args: Array[String]): Unit = {
    ...
    value.groupBy(_._1._1).map {
      case (key, value) =>
        val clickNum: Long = value.getOrElse((key, "click"), 0L) + 1L
        val orderNum: Long = value.getOrElse((key, "order"), 0L) + 1L
        val payNum: Long = value.getOrElse((key, "pay"), 0L) + 1L
...
class Myacc2 extends AccumulatorV2[UserVisitAction, mutable.Map[(String, String), Long]] {
  val map: mutable.Map[(String, String), Long] = mutable.Map[(String, String), Long]()

  override def isZero: Boolean = map.isEmpty

  override def copy(): AccumulatorV2[UserVisitAction, mutable.Map[(String, String), Long]] = new Myacc2
